...
* Description: Implements postion hold for pollinating single waypoint(flower).  
"""
...
class DroneFly:
    ...
    def __init__(self):
        ...
        )  # To get no. of red flowers from /red
    ...
    """
        ...
        * Input: None(self)
        * Output: Publishes rcPitch,rcRoll,rcThrottle,rcYaw for position hold
    ...
    def position_hold(self):
        ...
        while True:
            ...
            # Logic to break position_hold when a waypoint is reached
            if self.flag == 0:
                if (
                    ((self.Terror < erLimit + 0.2) & (self.Terror > -erLimit - 0.2))
            ...
            self.cmd.rcPitch = self.limit(pitch_value, 1800, 1200)

            roll_value = int(1500 + self.correct_roll)
            self.cmd.rcRoll = self.limit(roll_value, 1800, 1200)
    ...
    """
        ...
        * Input: data
        * Output: Sets redValue for no. of red flowers
        * Logic: Takes value from data.data and assigns it to redValue.         
        * Example Call: set_yaw(data)
    """

    def set_red(self, data):  # Provides the current red value from /red
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        temp.wp_x = -4.6
        temp.wp_y = 0.3
        temp.wp_z = 16.0 - 3  # minus 3 for reaching a little above the flower(waypoint)
        print (temp.wp_z)
        n = 3

        # Logic for successful pollination
        # After reaching a little above flower, drone decreases its height and continuously checks for pollination
        # If even after reaching the correct height(wp_z) pollination is unsuccessful, drone again increases its height and repeats process
        while True:
            temp.position_hold()
            if temp.redValue == 1:
                print ("Pollinated")
                break  # breaks out of loop on successful pollination
            if n == 0:
                n = 3
        ...
        # Increasing height of drone before changing x and y co-ordinates to avoid crashing
        temp.wp_x = -4.6
        temp.wp_y = 0.3
        temp.wp_z = 14.0
        temp.position_hold()
        ...
        # Increasing value of wp_z (decreasing height) in steps to avoid overshoot
        temp.wp_x = 0.0
        temp.wp_y = 0.0
        temp.wp_z = 19.0
        n = 5
        ...
        # Final Waypoint for position hold
        temp.flag = 1
        temp.wp_x = 0
        temp.wp_y = 0
        temp.wp_z = 25
